...
.L cl95cms.c++
 ...
 Updated by John Hakala; v1.31 May 11, 2016 - update for compatibility with ROOT6
-------------------------------------------------------------------------------- */
#include "Riostream.h"
//#include <stdlib.h>
#include "TMath.h"
...
//		if (plot) cout << "delta = " << delta << endl; 
	...
	//Double_t Norm = like.Integral(0.,xmax,p,epsilon);    //Fix for ROOT6
  like.SetParameters(p);
	...
	x2 = xmax;
	xmax = (x1 + x2)/2.;
	//delta = like.Integral(0,xmax,p,epsilon)/Norm - 0.95;   //Another fix for ROOT6
	...
	while (fabs(delta) > epsilon) 
	{
		...
		xmax = (x1 + x2)/2.;
		delta = like.Integral(0,xmax,epsilon)/Norm - 0.95;  //ROOT6
		if (timeout++ > 100) 
		{
			cout << "Maximum number of iterations reached (" << timeout << "); the following precision has been achieved: " << delta << endl;
			delta = 0.;
...
// (N is a global parameter. The parameter p is dummy).
{
	...
	if (sB == 0.)
	{
		...
		else 
		{
			...
			if (I == 0)
			{
				...
				//retval = in->Integral(low,A0+5.*sA,par,epsilon);  // ROOT6
	...
	else
	{
		...
		if (I == 0)
		{
			...
			//retval = out->Integral(low,B0+5.*sB,x,epsilon);    //ROOT6
      out->SetParameters(x);
			...
			while (tmp > epsilon)
			{	 
				//tmp = out->Integral(low,high,x,epsilon);  //ROOT6
        out->SetParameters(x);
...
// This function can make use of function Inner, which also takes into account uncertainty in lumi*effi.
{
	...
	else 
	{
		...
		{
			Double_t low = A0 > 5.*sA ? A0 - 5.*sA : 0.;
			//retval = TMath::Gaus(x[0],B0,sB,kTRUE)*in->Integral(low,A0+5.*sA,par,epsilon);  //ROOT6
      in->SetParameters(par);
      retval = TMath::Gaus(x[0],B0,sB,kTRUE)*in->Integral(low,A0+5.*sA,epsilon);
		} else {
			Double_t low = 0., high = A0 + 5.*sA, tmp = 1.;
			while (tmp > epsilon)
			{	 
