...
def getMCbgSampleKfactors():
  c = 0.9
  sampleXsects = {}
  sampleXsects[   "gJets100To200.root"   ]   = 1.6*c
  sampleXsects[   "gJets200To400.root"   ]   = 1.6*c
  sampleXsects[   "gJets400To600.root"   ]   = 1.4*c
  sampleXsects[   "gJets600ToInf.root"   ]   = 1.0*c
  sampleXsects[   "qcd300to500.root"     ]   = .7*c 
  sampleXsects[   "qcd500to700.root"     ]   = .7 *c
  sampleXsects[   "qcd700to1000.root"    ]   = .7 *c
  sampleXsects[   "qcd1000to1500.root"   ]   = .7 *c
  sampleXsects[   "qcd1500to2000.root"   ]   = .7 *c
  sampleXsects[   "qcd2000toInf.root"    ]   = .7 *c
  sampleXsects[   "qcd200to300.root"     ]  = 1*c
  sampleXsects[   "dyJetsQQ-180.root"    ]  = 1.23*c  
  sampleXsects[ "wJets600toInf.root" ] = 1*c
  sampleXsects[ "wJetsQQ-180.root" ]        = 1.21*c
  return sampleXsects
...
def getMCbgSampleXsects():
  ...
  #TODO: NEED TO FIND CORRECT CROSS-SECTION
  sampleXsects[ "wJets600toInf.root" ] = 80.0
  return sampleXsects
...
def getSmall3ddTreeDict(ddDir):
  ...
  s3dd[ "wJets600toInf.root"] = "%s/ddTree_wJetsToQQ_HT-600toInf"  % ddDir
